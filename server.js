...
const SELECTORS = {
  ...
  mainFrame: "#mainFrame",
  pnrStatusElement:
    ".PNRDisplayReference > #instanceCtrlContent_ctrlEnteteSyntheseDossier_lblEtatDossier",
  listOfClientMails:
    "#instanceCtrlContent_ctrlEnteteSyntheseDossier_hlEmailClient",
  textAreaSendConfirmationEmissions:
    "#instanceCtrlContent_ctrlEnvoiEmail_txtEmailDestinataire",
  sendEmailButton: "#instanceCtrlContent_ctrlEnvoiEmail_btEnvoyerEmail",
  confirmationEmailButton:
    ".ActionsListButtons > #instanceCtrlContent_btConfirmationParMail",
  printPNRReceiptButton:
    ".ActionsListButtons > #instanceCtrlContent_btImprimer",
  checkBoxPNRReceipt: "#instanceCtrlContent_btEnvoyerMail",
  sendReceiptPnrButton: "#instanceCtrlContent_btEnvoyerMail",
  textAreaSendPNRReceipt:
    "#instanceCtrlContent_ctrlEnvoiEmail_txtEmailDestinataire",
...
app.post("/send", async (req, res) => {
  ...
  async function sendPnrPDFMail(page) {
    try {
      ...
      const pnrStatusElement = await mainFrame.locator(
        SELECTORS.pnrStatusElement
      );
      ...
      if (PNR_NOT_AVAILABLE.includes(String(pnrStatusText))) {
        ...
        const textAreaSendConfirmationEmissions = await mainFrame.locator(
          SELECTORS.textAreaSendConfirmationEmissions
        );
  ...
  async function sendReceiptPnrMail(page) {
    ...
    const textAreaSendReceiptMail = await mainFrame.locator(
      "#instanceCtrlContent_ctrlEnvoiEmail_txtEmailDestinataire"
    );
    ...
    await mainFrame
      .locator("#instanceCtrlContent_ctrlEnvoiEmail_btEnvoyerEmail")
      .click();
